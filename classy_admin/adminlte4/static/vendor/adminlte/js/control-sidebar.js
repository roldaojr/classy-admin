...
class ControlSidebar {
  ...
  collapse() {
    ...
    if (this._config.controlSidebarSlide) {
      ...
      setTimeout(() => {
        const controlSidebar = document?.querySelector(this._config.target);
        if (controlSidebar) {
  ...
  toggle() {
    ...
    const shouldClose =
      body.classList.contains(CLASS_NAME_CONTROL_SIDEBAR_OPEN) ||
      body.classList.contains(CLASS_NAME_CONTROL_SIDEBAR_SLIDE);
    const shouldToggle =
      notVisible &&
      (body.classList.contains(CLASS_NAME_CONTROL_SIDEBAR_OPEN) ||
        body.classList.contains(CLASS_NAME_CONTROL_SIDEBAR_SLIDE));
  ...
  _init() {
    ...
    if (shouldNotHideAll) {
      ...
      const controlSidebars = document.querySelectorAll(
  ...
  _isNavbarFixed() {
    const { body } = document;
    return (
      body.classList.contains(CLASS_NAME_NAVBAR_FIXED) ||
      body.classList.contains(CLASS_NAME_NAVBAR_SM_FIXED) ||
      body.classList.contains(CLASS_NAME_NAVBAR_MD_FIXED) ||
      body.classList.contains(CLASS_NAME_NAVBAR_LG_FIXED) ||
      body.classList.contains(CLASS_NAME_NAVBAR_XL_FIXED)
    );
  }
  ...
  _isFooterFixed() {
    const { body } = document;
    return (
      body.classList.contains(CLASS_NAME_FOOTER_FIXED) ||
      body.classList.contains(CLASS_NAME_FOOTER_SM_FIXED) ||
      body.classList.contains(CLASS_NAME_FOOTER_MD_FIXED) ||
      body.classList.contains(CLASS_NAME_FOOTER_LG_FIXED) ||
      body.classList.contains(CLASS_NAME_FOOTER_XL_FIXED)
    );
  }
  ...
  _fixScrollHeight() {
    ...
    const controlSidebarContent = controlSidebar.querySelector(
    ...
    if (positions.top === 0 && positions.bottom === 0) {
      ...
      controlSidebarContent.style.height = `${
  ...
  _fixHeight() {
    return;
    const { body } = document;
    const controlSidebarContent = document.querySelector(
      `${this._config.target} ${SELECTOR_CONTROL_SIDEBAR_CONTENT}`
    );

    if (
      !controlSidebarContent ||
      !body.classList.contains(CLASS_NAME_LAYOUT_FIXED)
    ) {
      return;
    }
